...
snp_dists_path3 <- "analysis/snippy/ANU_overlap/snp_dists/ESC_NA4045AA_AS.pairwise_snps.csv"
...
snp_process(snp_dists_path2, snp_name2, refname2)
snp_process(snp_dists_path3, snp_name3, refname3)
...
ESC_NA4045AA_AS <- ESC_NA4045AA_AS %>% select(ESC_NA4045AA_AS, pair)

snp_df <- left_join(snp_df, ESC_NA4045AA_AS)
...
paired <- snp_df %>% select(HC400_1, HC200_1, HC100_1, HC50_1, HC20_1, HC10_1, HC5_1, HC2_1, HC0_1, pSF_088_nores_1, pMLST_1, working_name1, Source1, Country1, Year1, Classification1,
                            AVC171_chr, ST95_all_core_gene_alignment, ESC_NA4045AA_AS,
                            Classification2, Year2, Country2, Source2, working_name2, pMLST_2, pSF_088_nores_2, HC0_2, HC2_2, HC5_2, HC10_2, HC20_2, HC50_2, HC100_2, HC200_2, HC400_2)

paired <- paired %>% filter(working_name1 != working_name2)


human_non <- paired %>% filter(Source1 == "Human") %>%
        filter(Source2 != "Human") %>% filter(ST95_all_core_gene_alignment < 20) %>% arrange(ESC_NA4045AA_AS)

human_non <- human_non %>% rename('Phylogeny A (AVC171-ref SNP)' = AVC171_chr,
                                  'Phylogeny B (Pseudoreference-based, 99% core)' = ST95_all_core_gene_alignment,
                                  'Phylogeny C (M638866-ref SNP [Small, clonal cohort])' = ESC_NA4045AA_AS)


write_csv(human_non, "supplementary_tables/Supplementary_Table_1.csv")
